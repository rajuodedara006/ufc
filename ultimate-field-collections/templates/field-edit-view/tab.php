<?php
$displayed_fields = array( 'ID', 'type', 'key', 'label', 'name', 'endpoint', 'parent', 'menu_order', '_name', '_valid', 'conditional_logic' );
	...
	<div class="field-edit-content-main">
		<div class="field-edit-content-row">
			<div class="field-edit-col">
				...
				<?php
				$field_name = !empty($field_name) ? $field_name : sanitize_title($field_label) . '_tab';
				?>
	    ...
	    <div class="field-edit-content-row">
	      <div class="field-edit-col">
	        <label class="ufc-field-checkbox-switch" for="acf_fields[<?php esc_attr_e($field_ID); ?>][endpoint]">
	          ...
	          <p class="description"><?php esc_html_e('Define an endpoint for the previous tabs to stop. This will start a new group of tabs.', "ufcsupport" ); ?></p>
			...
			if ( is_array($logic_group_arr) ) {
				foreach ($logic_group_arr as $logic_no => $logic_arr) {
					if ( isset($logic_arr['field']) && isset($logic_arr['operator']) ) {
						echo '<input type="hidden" name="acf_fields['.esc_attr__($field_ID).']['.esc_attr__($logic_group_no).']['.esc_attr__($logic_no).'][field]" value="'.esc_attr__($logic_arr['field']).'">';
						echo '<input type="hidden" name="acf_fields['.esc_attr__($field_ID).']['.esc_attr__($logic_group_no).']['.esc_attr__($logic_no).'][operator]" value="'.esc_attr__($logic_arr['operator']).'">';
					}
				}
			}
		}
