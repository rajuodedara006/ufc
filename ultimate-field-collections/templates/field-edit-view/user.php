...
<div class="field-edit-view-wrap field-type-<?php esc_attr_e($field_type); ?>"
    ...
    <div class="field-edit-content-main">
        ...
        <div class="field-edit-content-row">
            <?php $displayed_fields[] = 'role'; ?>
            <div class="field-edit-col">
                <label class="field-label"
                    id="acf_fields[<?php esc_attr_e($field_ID); ?>][role][]"><?php esc_html_e('Filter by role', "ufcsupport" ); ?></label>
                <input type="hidden" name="acf_fields[<?php esc_attr_e($field_ID); ?>][role]"
                    id="acf_fields-<?php esc_attr_e($field_ID); ?>-role-input">
                <select class="ufc-field-select ufc-field-select2"
                    name="acf_fields[<?php esc_attr_e($field_ID); ?>][role][]"
                    id="acf_fields[<?php esc_attr_e($field_ID); ?>][role][]"
                    data-placeholder="<?php esc_html_e('All user roles', "ufcsupport" ); ?>" multiple>
                    <?php
                    $roles = acf_get_user_role_labels();
					if ( !empty($roles) ) {
						foreach ($roles as $role_key => $role) {
							if ( !empty($role) ) {
                                $selected = ( !empty($field_data['role']) && in_array($role_key, $field_data['role']) ) ? 'selected' : '' ;
                                echo '<option value="'.esc_attr($role_key).'" '.esc_attr($selected).'>'.esc_html__($role).'</option>';
							}
						}
					}
					?>
