		...
		<div class="acf-checkbox-wrap">
			...
			if ( !empty($field_data['choices']) ) {
				...
				foreach ($field_data['choices'] as $key => $value) {
					$number++;
					$checked = ( is_array($field_val) && (in_array($key, $field_val)) ) ? 'checked="checked"' : '';

					echo '<label class="acf-checkbox-choices"><input id="acf-'.esc_attr($field_key).'-Option-'.esc_attr($number).'" type="checkbox" name="acf['.esc_attr($field_key).']['.esc_attr($number).']"  value="'.esc_attr($key).'" '.esc_attr($checked).'>'.esc_html__($value).'</label>';
				}
			}
			?>
		</div>
